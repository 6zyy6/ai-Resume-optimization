...
export default function NewJobPage() {
  ...
  async function parseJob() {
    ...
    await createWebApiClient().post(`/v1/jobs/${job.id}/parse`, {}, crypto.randomUUID());
    setJobId(job.id);
    setParsed(true);
  }
  async function matchJob() {
    ...
    const analysis = await createWebApiClient().post<typeof body, components["schemas"]["MatchResponse"]>("/v1/match-analyses", body, crypto.randomUUID());
